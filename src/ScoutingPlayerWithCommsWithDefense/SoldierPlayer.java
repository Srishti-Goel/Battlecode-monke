...
public class SoldierPlayer {
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        ...
        if(soldier_mode == 1){
            rc.setIndicatorString("Defending random");
            ...
            if(archonsFound == 0) {
                ...
                if(rc.getLocation().distanceSquaredTo(toAttack) < rc.getType().visionRadiusSquared){
                    ...
                    if(enemyArchon.type != RobotType.ARCHON || enemyArchon.team != rc.getTeam().opponent()){
                        archonsFound--;
                        MapLocation copyingLoc = new MapLocation(rc.readSharedArray( 2 + (archonToAttack *2)), rc.readSharedArray( 3 + (archonToAttack *2)));
